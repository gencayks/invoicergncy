-- Create a stored procedure to create the invoice_drafts table
CREATE OR REPLACE FUNCTION public.create_invoice_drafts_table()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND tablename = 'invoice_drafts'
  ) THEN
    -- Create the invoice_drafts table
...
GRANT EXECUTE ON FUNCTION public.create_invoice_drafts_table() TO authenticated;
